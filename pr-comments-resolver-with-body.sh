...
claude --model sonnet -p "Could you fix $COMMENT_URL on $PR_NUMBER? After fix it, commit and push changes. Then please add reaction to the comment. When replying to the comment, let me know the comment is written by ClaudeCode adding 'Generated With [Cluade Code](..)'." --from-pr $PR_NUMBER
