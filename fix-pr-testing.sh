...
claude --model sonnet -p "Fix workflow job error on $PR_URL PR. This is workflow run url( $RUN_URL ) . You should make sure that pass all playwright tests. Then, make and push changes." --from-pr $PR_NUMBER
